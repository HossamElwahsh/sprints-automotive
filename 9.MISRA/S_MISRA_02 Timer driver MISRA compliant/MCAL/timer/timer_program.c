...
/* Private Functions */
static en_timer_error_t timer_start(EN_ClockSelection_t enClockPrescaleSelection);
static void timer_setTimerValue(uint8_t_ timerInitValue);
 ...
 * @param uint16_desired_delay [in] (ms) delay to wait for - Range 2ms upto 60 seconds, sensitivity ~2 ms
 * @return Error if any
 */
void timer_ms_delay(uint16_t_ uint16_desired_delay)
{
    ...
    int overflowsCount = ceil(uint16_desired_delay / maxDelay); // in counts #

    // desired delay wait loop
    int currentOverflowNumber = 0;

    // Calculate Timer Initial value to achieve desired delay
    uint8_t_ timerInitValue = 0;
    if (overflowsCount > 1)
    {
//        timerInitValue = floor(pow(2, timerNBits) - ((uint16_desired_delay / oneTickTime) / overflowsCount)); // in decimal  (0 - 255)

        // using bitShift instead of power to reduce time
        timerInitValue = (uint8_t_)((1 << timerNBits) - ((uint16_desired_delay - (maxDelay * (overflowsCount - 1))) / oneTickTime)); // in decimal  (0 - 255)
    }
    else
    {
        timerInitValue = floor((maxDelay - uint16_desired_delay) / oneTickTime); // in decimal (0 - 255)
    }
    ...
    do{
        while((GET_BIT(TIFR, 0)) == 0); // wait for next overflow
