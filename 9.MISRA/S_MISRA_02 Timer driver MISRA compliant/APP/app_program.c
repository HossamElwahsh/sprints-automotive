...
#define BUTTON_0_PIN DIO_PIN_3 // INT1

#define BUTTON_1_PORT DIO_PORT_D
#define BUTTON_1_PIN DIO_PIN_2   // INT0
...
/* Global Variables */
uint8_t_ state_number = 0;
uint8_t_ current_blink_mode_index = 1;
...
void App_init()
{
    ...
    led_init(LEDS_PORT, LED_0_PIN);
    led_init(LEDS_PORT, LED_1_PIN);
    led_init(LEDS_PORT, LED_2_PIN);
    led_init(LEDS_PORT, LED_3_PIN);

    /* Initialize Button 0 on port D Pin 3 */
    btn_init(BUTTON_0_PORT, BUTTON_0_PIN);

    /* Initialize Button 1 on port D Pin 2 */
    btn_init(BUTTON_1_PORT, BUTTON_1_PIN);

    /* Enable external interrupts on INT0 (D-2) for (BUTTON_1) on the falling edge */
    EXI_enableInterrupt(EXI_INT0, EXI_SENSE_FALLING_EDGE);

    /* Enable external interrupts on INT1 (D-3) for (BUTTON_0) on the falling edge */
    EXI_enableInterrupt(EXI_INT1, EXI_SENSE_FALLING_EDGE);
...
void App_Start() {
    while (TRUE) {
        ...
        if(currentOnLEDs != 0)
        {
            led_array_blink(LEDS_PORT, currentBlinkMode.timeOn, currentBlinkMode.timeOff,
                            currentOnLEDs); // turn on first 1 LED
        }else{
            led_array_off(LEDS_PORT, 0); // turn off all LEDs
        }
